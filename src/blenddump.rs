...
pub fn dump(repo: &Path, pkg: &str, lang: &str, out: &Path) -> Result<()> {
    ...
    let mass = |select: fn(&crate::schema::SignalEdge) -> f64| {
        data.edges.iter().map(select).sum::<f64>()
    };
    let raw = json!({
    ...
    let below_prune_floor = blended.iter().filter(|w| **w < FLOOR).count() as f64
        / n_blended_edges.max(1) as f64;
