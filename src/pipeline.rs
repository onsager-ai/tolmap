...
pub fn blend(data: &mut GraphData) -> Result<()> {
    ...
    let raw_static = data
        ...
        .sum::<f64>();
    let raw_cochange = data
        .edges
        .iter()
        .map(|edge| edge.cochange)
        .sum::<f64>();
    let raw_proximity = data
        .edges
        .iter()
        .map(|edge| edge.proximity)
        .sum::<f64>();
    let raw_semantic = data
        .edges
        .iter()
    ...
    let raw_cochange = if raw_cochange == 0.0 { 1.0 } else { raw_cochange };
    let raw_proximity = if raw_proximity == 0.0 { 1.0 } else { raw_proximity };
    let raw_semantic = if raw_semantic == 0.0 { 1.0 } else { raw_semantic };
    ...
    ensure!(maximum.is_finite() && maximum > 0.0, "invalid blended edge mass");
    for edge in &mut data.edges {
        edge.weight /= maximum;
    }
    Ok(())
...
fn weighted_graph(data: &GraphData) -> Result<WeightedGraph> {
    ...
    for edge in &data.edges {
        edges.push(WeightedEdge {
            a: *index.get(edge.a.as_str()).ok_or_else(|| {
                anyhow::anyhow!("edge references missing file {}", edge.a)
            })?,
            b: *index.get(edge.b.as_str()).ok_or_else(|| {
                anyhow::anyhow!("edge references missing file {}", edge.b)
            })?,
            weight: edge.weight,
        });
    }
...
fn layout(
    ...
    let district_count = membership.iter().copied().max().map_or(0, |value| value + 1);
...
pub(crate) fn force_layout(
    ...
    let mut points = (0..count)
        .map(|index| {
            let angle = (index as f64 * 2.399_963_229_728_653 + seed as f64 * 0.17)
                % std::f64::consts::TAU;
...
fn normalize_points(points: &[[f64; 2]]) -> Vec<[f64; 2]> {
    ...
    let x_min = points.iter().map(|point| point[0]).fold(f64::INFINITY, f64::min);
    let x_max = points
        .iter()
        .map(|point| point[0])
        .fold(f64::NEG_INFINITY, f64::max);
    let y_min = points.iter().map(|point| point[1]).fold(f64::INFINITY, f64::min);
    let y_max = points
        .iter()
        .map(|point| point[1])
        .fold(f64::NEG_INFINITY, f64::max);
    ...
    while index < values.len() {
        ...
        if width >= height {
            ...
            for (item, value) in row {
                let row_height = if sum == 0.0 { 0.0 } else { value / sum * height };
...
fn worst(row: &[(usize, f64)], length: f64) -> f64 {
    ...
    (length * length * maximum / (sum * sum))
        .max(sum * sum / (length * length * minimum))
}
...
fn landmarks(
    ...
    for index in hubs.into_iter().take(2) {
        add_landmark(
            ...
            index,
            "hub",
            format!("fan-in {}", python_number(data.nodes[index].fanin, &data.lang)),
        );
    }
