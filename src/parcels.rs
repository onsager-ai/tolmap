...
pub fn build_parcels(document: &MapDocument) -> BTreeMap<String, Vec<[f64; 2]>> {
    ...
    for (district, members) in groups {
        ...
        for point in points
            .iter()
            .chain(district_geometry.blob.iter().flatten())
        {
        ...
        let span = (high[0] - low[0] + padding)
            ...
            .max(1e-9);
        let grid = GRID.max(((members.len() as f64).sqrt() * 26.0) as usize).min(320);
...
fn power_cells(
    ...
    for iteration in 0..ITERATIONS {
        ...
        let errors = area
            ...
            .collect::<Vec<_>>();
        if errors.iter().map(|value| value.abs()).fold(0.0_f64, f64::max) < 0.08 {
            break;
        }
...
fn pearson(left: &[f64], right: &[f64]) -> Option<f64> {
    ...
    let numerator = left
        ...
        .sum::<f64>();
    let left_norm = left.iter().map(|value| (value - left_mean).powi(2)).sum::<f64>();
