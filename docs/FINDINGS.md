...
[Paired standard-runner Actions builds](https://github.com/onsager-ai/tolmap/actions/runs/35890831908) used this branch at `983fda22e7bbc11536be1b96840a3d8bf56e90cb` and `main` on each repository's pinned `eval/corpus.toml` commit. All eight builds succeeded. The `eval/symbol_stats.py` metrics were written by Actions into each result artifact. “r” is the median of per-file Pearson correlations between top-level card area and symbol code lines; a file needs at least three top-level cards and variation in both values. Sizes are raw compact JSON decimal MB, before and after geometry. The alternative size applies the prototype's ×1e4 delta coding to the *same* rings, as a size comparison only.
