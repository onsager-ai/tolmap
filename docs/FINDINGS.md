...
**"No build" is not something the tool offers.** Checked against the pinned rust-analyzer release's (`2026-09-21`, linux-gnu asset SHA256 `b2d24ce2…8257d0`) own `flags.rs`: `diagnostics`, `analysis-stats`, `unresolved-references` and `prime-caches` all have `--disable-build-scripts`/`--disable-proc-macros`; `scip` has neither. Its `run()` hardcodes `LoadCargoConfig { load_out_dirs_from_check: true, with_proc_macro_server: ProcMacroServerChoice::Sysroot, .. }` regardless of `--config-path`. The `config_probe` job below confirms this empirically, not just from source: an LSP-style `--config-path` JSON setting `cargo.buildScripts.enable: false` and `procMacro.enable: false` produced a **byte-identical** set of `execve`d binaries and the same 6,677,023-byte index as the plain run, differing in the log only by crate-download ordering (network is unordered per-connection) and one added `Config Error(s) error_sink=ConfigErrors([])` line confirming the file parsed with no complaint and simply changed nothing.

Every run — network and offline alike — spawns `cargo` as a child process (`strace -f -e trace=execve`, run links below); network runs on tolmap, ripgrep and tokio also spawn `ar`, `as` and `cc`, i.e. real native compilation of build scripts and/or proc-macro crates, not just metadata resolution.

### Network vs. offline: it does not need network, but degrades hard when denied it

Three repos, one pinned commit each: `onsager-ai/tolmap@c62a968`, `BurntSushi/ripgrep@0e8390a` (tag `14.1.1`) and `tokio-rs/tokio@bb9d570` (tag `tokio-1.42.0`). Each ran twice with a job-fresh, otherwise-empty `CARGO_HOME`: `network` (default runner egress) and `offline` (`CARGO_NET_OFFLINE=true`). `--exclude-vendored-libraries` on every run, scoping the index to each repo's own files.

| repo | variant | wall s | peak RSS MB | index MB | files indexed / tracked | documents | occurrences | definitions | file-pairs derivable | run |
|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---|
| tolmap | network | 64.3 | 1590 | 6.68 | 38 / 38 | 38 | 70,038 | 2,136 | 431 | [job](https://github.com/onsager-ai/tolmap/actions/runs/36222830603/job/108351181904) |
| tolmap | offline | 16.8 | 734 | 6.29 | 38 / 38 | 38 | 65,286 | 2,136 | 429 | [job](https://github.com/onsager-ai/tolmap/actions/runs/36222830603/job/108351181931) |
| ripgrep | network | 27.2 | 1031 | 7.42 | 97 / 98 | 97 | 72,050 | 4,990 | 544 | [job](https://github.com/onsager-ai/tolmap/actions/runs/36222830603/job/108351181959) |
| ripgrep | offline | 17.1 | 808 | 6.68 | 95 / 98 | 95 | 63,905 | 4,877 | 373 | [job](https://github.com/onsager-ai/tolmap/actions/runs/36222830603/job/108351182116) |
| tokio | network | 68.3 | 2055 | 17.60 | 617 / 720 | 617 | 158,713 | 9,154 | 82,203 | [job](https://github.com/onsager-ai/tolmap/actions/runs/36222830603/job/108351181976) |
| tokio | offline | 13.0 | 698 | 3.63 | 336 / 720 | 336 | 34,191 | 2,039 | 27,725 | [job](https://github.com/onsager-ai/tolmap/actions/runs/36222830603/job/108351181912) |
| tolmap | config-path probe | 52.2 | 1592 | 6.68 (identical bytes to network) | 38 / 38 | — | — | — | — | [job](https://github.com/onsager-ai/tolmap/actions/runs/36222830603/job/108351181773) |

Full run: [36222830603](https://github.com/onsager-ai/tolmap/actions/runs/36222830603) (dispatched via `remote-build.yml`'s new `command=rust-scip-spike`, since `rust-scip-spike.yml` cannot itself be `workflow_dispatch`-targeted until it is on `main` — the same limitation finding 41's spike had). Wall time and RSS are `/usr/bin/time -v`; "documents" and "file-pairs derivable" are `eval/scip_ingest.py`'s `ingest()`, unmodified.

**Offline does not fail — it silently downgrades.** `cargo metadata` itself fails offline on all three repos (`error: no matching package named 'anyhow'/'aho-corasick'/'bytes' found ... location searched: crates.io index`, exit 101), because a fresh `CARGO_HOME` has no registry index cached and none of Cargo.lock's pins can be re-verified against it. rust-analyzer catches that failure itself and retries with `cargo metadata --no-deps`, logging `WARN 'cargo metadata' failed and returning succeeded result with '--no-deps'`, then proceeds to index whatever it can with no resolved dependency graph at all — not even path dependencies between a workspace's own member crates, as tokio's numbers show. `CARGO_HOME` afterward held only cargo's own skeleton config (57,521 bytes, 0 extracted registry crate dirs) on every offline run, against 39.7–181.3 MB and (the more informative number, since the directory-count script under-measured — see "Not verified") real per-crate registry downloads on every network run: `grep -c Downloaded` on the network logs counts 160 crates fetched for tolmap, 38 for ripgrep, 145 for tokio.

**How much the degradation costs depends entirely on how much of the crate lives outside one file.** tolmap (single crate, no workspace, 38 files) loses almost nothing: definitions are identical (2,136 = 2,136) and file-pairs derivable drop by only 2 of 431 (0.5%) — cross-crate resolution barely matters when there is effectively one crate. ripgrep (a small multi-crate workspace) loses 31.4% of its derivable file pairs (544 → 373) and 2 more files go unindexed (95/98 vs 97/98). tokio (a large, heavily feature-cfg-gated multi-crate workspace) loses the most: files indexed drops from 617/720 (85.7%) to 336/720 (46.7%), and file-pairs derivable from 82,203 to 27,725 (a 66.3% loss). A no-install worker, per #126's framing, would be measuring a materially smaller graph than an installed one on exactly the repos this matters most for — the same shape finding 41 found for TypeScript monorepos without `pnpm install`, now confirmed for Rust's own dependency resolution rather than a language server's project-config loading.

**Feature/platform cfg-gating gets worse offline, and for a reason the sampler didn't originally cover.** tokio's network run drops exactly the one file a Linux runner's default target should drop — `tokio/src/signal/windows.rs` — of 134 files this finding's regex sample flagged as carrying a `cfg(windows|unix|target_os|target_family)` gate in their first 15 lines. Its offline run drops 13 of those 134, including `tokio/src/net/tcp/socket.rs`, `tokio/src/io/interest.rs` and `tokio/src/signal/unix.rs` — files that are *not* Windows-only and indexed fine under network. The likely cause is downstream of the same `--no-deps` degradation: without a resolved dependency graph, rust-analyzer cannot reliably compute which Cargo features are active, and several of tokio's modules are behind `#[cfg(feature = "...")]` on the `mod` declaration in a different file, which this sampler's file-local regex cannot see (it only reads the first 15 lines of the *candidate* file itself, not the file that declares `mod candidate;`). ripgrep's one network-unindexed file, `crates/globset/src/serde_impl.rs`, and its two additional offline-unindexed files, `crates/printer/src/{json,jsont}.rs`, are the same class of miss: all three are feature-gated (`serde1`, `json`) from their crate's `lib.rs`, not from a `#[cfg]` inside themselves, so `cfg_gated_and_unindexed` in the JSON output is `[]` for both ripgrep variants even though the exclusion is real.

### What it loses: no evidence of proc-macro expansion in any sample, in any variant

`eval/rust_scip_stats.py` samples files carrying at least one non-builtin `#[derive(...)]` and checks the network run's index for a symbol matching that macro's known generated-method markers (e.g. `Parser` → `augment_args`/`from_arg_matches`, `TS` → `decl`/`inline`/`name`, `Serialize`/`Deserialize` → `serialize`/`deserialize`). Across 12 samples on tolmap (`Parser`, `Subcommand`, `Serialize`, `Deserialize`, `TS`, on `Cli`, `Command`, `District`, `SymbolRow`, `ProgressValue`, `StageId`, `NamingContext`, `SpendLedger`, `CacheEntry`, `PythonImport` and `Args`) and 10 on ripgrep (`Deserialize` on `tests/json.rs`'s ten JSON-fixture structs), every verdict was **"not observed"**: the marker method never appears among that type's indexed symbols, even though the network run did compile native code for build scripts and/or proc-macro crates (`cc`/`ar`/`as` in the `execve` trace). tokio yielded zero samples: at this commit it has no `build.rs` in its tracked files and no `#[derive(...)]` this sampler's stride (every ~7th of 720 files, capped at 12 hits) turned up a non-builtin derive on — its own macros (`#[tokio::main]`, `pin_project_lite!`) are attribute/declarative, not `#[derive(...)]`, so this sampler's regex does not apply to them at all. tolmap's own build.rs (`native/leiden_bridge.cpp` via `cc::Build`) has no Rust source reading its `OUT_DIR`, so there was nothing to check there, and it did not block or degrade the index — `LEIDEN_PREFIX` was never set on these runners and no library was installed, and every run still succeeded at exit 0. That is itself informative for #126: a build script that would panic under an actual `cargo build` (tolmap's does, without `LEIDEN_PREFIX`) does not visibly break `rust-analyzer scip`'s `load_out_dirs_from_check` step; the OUT_DIR-priming failure, if any, is swallowed the same way the offline `cargo metadata` failure is.

### What this settles and what it does not

It settles:
- The `scip` subcommand cannot be told to skip build scripts or proc macros; `--config-path` does not change its behaviour, confirmed both from source and from a byte-identical index.
- It does not hard-require network — `CARGO_NET_OFFLINE` plus an empty `CARGO_HOME` still produces an index on all three repos — but the no-network path is a silent, uneven degradation, not a clean "no-install" mode like Python's or Go's in finding 41. How much it costs scales with how multi-crate and feature-cfg-heavy the target is, from ~0% (tolmap) to ~66% of file-pairs (tokio).
- No sampled third-party derive macro's generated methods were visible in the index, in any variant, including one where the compiler was demonstrably invoked. This is a marker-based proxy, not a full accounting (see below), but 22/22 samples agreeing is a real signal, not noise.

It does not settle:
- Whether a *warm*, network-enabled, pre-populated `CARGO_HOME` (a real worker's steady state, not a job-fresh one) resolves everything the network runs above still missed — 617/720 and 97/98 are not 100%, and this finding did not investigate the remainder file by file.
- Whether `ProcMacroServerChoice::Sysroot` ever expands anything for a project's *own* macros (as opposed to std's built-in derives) under any configuration; this finding shows it does not do so visibly for the samples tried, not that it is incapable of it.
- rust-lang/rust-analyzer itself, the large-workspace candidate #126 floated, was not indexed; tokio stands in for "large, multi-crate, heavily cfg-gated" here.
- CVE/toolchain risk of running an untrusted repository's build scripts and proc-macro crates as native code during indexing — the network runs did exactly that (`cc`/`ar`/`as`), which is a sandboxing question #110's P1c design (`docs/SCIP_SANDBOX.md`) already covers for JS/TS installs and would need to cover for Rust too, worse here since it is arbitrary compiled code, not just an npm lifecycle script.

### Not verified / known measurement gaps
- `cargo_home_crates.txt`'s directory count is wrong by construction (`find "$CARGO_HOME/registry/src" -mindepth 1 -maxdepth 1` counts the one source-id directory, not per-crate subdirectories one level below it) and was not fixed before this PR's numbers were taken. `cargo_home_bytes.txt`'s total-size numbers and the log's own `Downloaded` counts are unaffected and are what this finding relies on.
