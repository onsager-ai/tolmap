...
| `TOLMAP_REFS` | `hand` | reference graph for job maps: `hand` (tree-sitter resolver) or `scip` (issue #110: the SCIP indexers from the `TOLMAP_SCIP_*` rows below, with a per-language fallback to `hand` -- including when an indexer is not installed -- recorded in the map's `coverage.references`). The default is shared with `tolmap build --refs`; unset or invalid uses `hand` |
...
`null` once running and in terminal states. For queued jobs, `eta_start_s` is the predicted wait in seconds, from a simulation of the dispatcher: each slot is free after the estimated remaining time of its running job (now, if idle), and each queued job in FIFO order starts on the slot that frees first, which is then busy for that job's estimated runtime (`docs/WORKER_TIER.md` §7.2). With one slot (`TOLMAP_MAX_CONCURRENT_JOBS=1`, the default) that is the sum of the running job's remaining time and the runtimes of the jobs ahead, exactly as before; with more slots it is no longer the sum over all of them. It updates as jobs ahead progress; it is null once running or terminal. `eta` is a remaining-time range for the job itself. The model starts from finding 36 and corpus timings, refits from completed job stages in SQLite, and blends current-stage progress with an EWMA rate. The interval widens when file count exceeds observed training sizes. Either field may be null when unknown. No wall-time timeout is imposed. `elapsed_s`
