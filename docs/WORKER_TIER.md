...
- **The commit is not pinned yet** (§3.3): a loopback agent builds what its clone resolves HEAD to, as local mode does, and checks the child's commit against its own checkout before uploading. The master takes the agent's commit and does not yet compare it, or `files` and `districts`, with the pinned commit and the documents (§5.4); both belong with the first remote worker.
