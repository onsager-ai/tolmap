...
- **834 parcels holding 2,430 files (66%).** 579 are single files; the largest holds 51. **827 of 834 (99.2%) lie inside a single vendor directory**, so a grid ordered by address is a grid ordered by integration. 254 vendor directories appear as parcels; 32 of them span more than one parcel (a v1 and a v2 of the same node with no shared file).
...
3. **The locale plat.** django d0 decomposes into one 31-file sub-district and 171 parcels, each inside one `conf/locale/<lang>/` directory. If the locale files are not parcels, the terrain test is wrong.
4. **The 308-plugin case.** `workflow/src/index.ts` is n8n d0's first arterial; at least 95% of its parcels (measured 99.2%) lie inside one vendor directory.
